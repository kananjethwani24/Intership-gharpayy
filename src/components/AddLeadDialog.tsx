...
import { Plus, AlertTriangle, Sparkles, Phone, Mail, MapPin, IndianRupee, User, StickyNote, Calendar, Briefcase, Clock } from 'lucide-react';
...
const AddLeadDialog = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setParsed(null);
      setRawText('');
      setForm({ name: '', phone: '', email: '', source: 'whatsapp', budget: '', preferred_location: '', notes: '', assigned_agent_id: '', movingDate: '', gender: '', occupation: '', stayDuration: '' });
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input placeholder="Date or Month" value={form.movingDate} onChange={e => setForm(f => ({ ...f, movingDate: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Stay Duration (Month)</Label>
              <Input placeholder="e.g. 6 Months" value={form.stayDuration} onChange={e => setForm(f => ({ ...f, stayDuration: e.target.value }))} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">Occupation (Student/Working)</Label>
