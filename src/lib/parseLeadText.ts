...
const KV_LOCATION_RE = /(?:^|\n)\s*(?:location|area|city|locality|place|sector)\s*[:=\-–]\s*(.+)/i;
const KV_ADDRESS_RE = /(?:^|\n)\s*(?:address|street|house|landmark)\s*[:=\-–]\s*(.+)/i;
const KV_DATE_RE = /(?:^|\n)\s*(?:moving\s*date|date|moving|joining)\s*[:=\-–]\s*(.+)/i;
const KV_GENDER_RE = /(?:^|\n)\s*(?:gender|sex)\s*[:=\-–]\s*(.+)/i;
const KV_OCCUPATION_RE = /(?:^|\n)\s*(?:occupation|student|working|job|profile|work)\s*[:=\-–]\s*(.+)/i;
const KV_DURATION_RE = /(?:^|\n)\s*(?:duration|month|stay|period)\s*[:=\-–]\s*(.+)/i;
const KV_ROOM_RE = /(?:^|\n)\s*(?:type|property|flat|room|bhk|requirement|looking\s*for|interested\s*in)\s*[:=\-–]\s*(.+)/i;

// Regex for free text
const GENDER_RE = /\b(male|female|boy|girl|man|woman|men|women|others?)\b/i;
const OCCUPATION_RE = /\b(student|working|professional|job|intern|self\s*employed)\b/i;
const DATE_KEYWORD_RE = /\b(?:moving|joining|starting|shifting|from)\s*(?:on|date)?\s*[:=]?\s*((?:\d{1,2}[/.-]\d{1,2}[/.-]\d{2,4})|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\s*\d{0,2}|(?:next|this)\s*(?:month|week|monday|tuesday|wednesday|thursday|friday|saturday|sunday))\b/i;
const DURATION_RE = /\b(\d+\s*(?:month|yr|year|day)s?|long\s*term|short\s*term)\b/i;
...
export function parseLeadText(raw: string): ParsedLead {
  ...
  // 6. Extract Gender
  let gender: ParsedLead['gender'] = '';
  const genderMatch = text.match(GENDER_RE);
  if (genderMatch) {
    const g = genderMatch[1].toLowerCase();
    if (g === 'male' || g === 'boy' || g === 'man' || g === 'men') gender = 'Male';
    else if (g === 'female' || g === 'girl' || g === 'woman' || g === 'women') gender = 'Female';
    else if (g === 'other' || g === 'others') gender = 'Other';
    extracted.push(escapeRegex(genderMatch[0]));
  }

  // 7. Extract Occupation
  let occupation: ParsedLead['occupation'] = '';
  const occMatch = text.match(OCCUPATION_RE);
  if (occMatch) {
    const o = occMatch[1].toLowerCase();
    if (o === 'student' || o === 'intern') occupation = 'Student';
    else if (o === 'working' || o === 'professional' || o === 'job' || o.includes('employed')) occupation = 'Working';
    extracted.push(escapeRegex(occMatch[0]));
  }

  // 8. Extract Moving Date
  let movingDate = '';
  const dateMatch = text.match(DATE_KEYWORD_RE);
  if (dateMatch) {
    movingDate = dateMatch[1].trim();
    extracted.push(escapeRegex(dateMatch[0]));
  }
  ...
  if (durationMatch) {
    ...
    extracted.push(escapeRegex(durationMatch[0]));
  }

  // 10. Remove extracted tokens to find name and notes
  let remaining = text;
...
function parseKeyValue(text: string): ParsedLead {
  ...
  const addrM = text.match(KV_ADDRESS_RE);
  if (addrM) { result.address = addrM[1].trim(); }

  const dateM = text.match(KV_DATE_RE);
  if (dateM) { result.moving_date = dateM[1].trim(); }

  const genM = text.match(KV_GENDER_RE);
  if (genM) {
    const g = genM[1].toLowerCase();
    if (g.includes('male') || g.includes('boy')) result.gender = 'Male';
    else if (g.includes('female') || g.includes('girl')) result.gender = 'Female';
  }

  const occM = text.match(KV_OCCUPATION_RE);
  if (occM) {
    const o = occM[1].toLowerCase();
    if (o.includes('student')) result.occupation = 'Student';
    else if (o.includes('work')) result.occupation = 'Working';
  }

  const durM = text.match(KV_DURATION_RE);
  if (durM) { result.stay_duration = durM[1].trim(); }
...
function empty(): ParsedLead {
  return {
    name: '', phone: '', email: '', budget: '', preferred_location: '', address: '',
    moving_date: '', gender: '', occupation: '', stay_duration: '', notes: '',
