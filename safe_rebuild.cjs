...
const head = lines.slice(0, 371);
const body = `  );
}

function PropertyDialog({ selectedProperty, onClose }) {
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto p-0 border-none rounded-[2rem] gap-0">
        ...
        <div className="bg-white overflow-hidden flex flex-col">
          <div className="bg-slate-900 px-8 py-10 relative">
            <div className="absolute top-0 right-0 w-80 h-80 bg-slate-800 rounded-full blur-[80px] -translate-y-1/2 translate-x-1/2 opacity-50" />
            <div className="relative z-10 flex flex-col md:flex-row md:items-start justify-between gap-6">
              ...
              <Badge className="py-1.5 px-4 text-[10px] font-black uppercase tracking-widest bg-emerald-500 text-white">{selectedProperty.gender || 'Any'}</Badge>
            </div>
          </div>
          ...
          <div className="p-8 grid grid-cols-1 md:grid-cols-12 gap-10">
            ...
            <div className="md:col-span-4 space-y-8">
               ...
               <div className=\"space-y-3\">
                  ...
                  ) : <Button variant=\"outline\" className=\"w-full h-12 rounded-xl border-slate-200\" onClick={() => uploadPdf(null)}>Attach Brochure PDF</Button>}
                  <Button variant=\"outline\" className=\"w-full h-12 rounded-xl border-slate-200\" onClick={() => window.open(selectedProperty.photosLink, '_blank')}>View Photos</Button>
               </div>
            </div>
          </div>
        </div>
