    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <button onClick={() => router.push('/explore')} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {similarProperties.slice(0, 3).map((sp: any) => (
                      <div
                        ...
                        onClick={() => router.push(`/property/${sp.id}`)}
                      >
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Bellandur', 'Brookefield', 'Whitefield', 'Marathahalli', 'Sarjapur Road', 'HSR Layout'].map(area => (
                    <Badge key={area} variant="secondary" className="cursor-pointer text-[11px]" onClick={() => router.push(`/explore?area=${area}`)}>
